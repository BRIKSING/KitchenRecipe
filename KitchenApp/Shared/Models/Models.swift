...
struct PaginatedResponse<T: Decodable>: Decodable {
    ...
    // Бэкенд возвращает { items, total, page, per_page, pages }.
    // Маппим `items` → `data` и вычисляем `hasMore` из номера страницы и общего числа страниц,
    // сохраняя публичный интерфейс (data / hasMore) для остальных этапов.
    enum CodingKeys: String, CodingKey {
        case items
        case total
        case page
