...
- [x] Создание Xcode-проекта (SwiftUI, iOS 17+)
- [x] Структура папок (Core / Features / Shared / Resources)
- [x] `APIClient` — базовый сетевой слой (URLSession + async/await)
- [x] `Endpoint` enum — все эндпоинты
- [x] Доменные модели (Recipe, Step, Ingredient …) + декодирование
- [x] Глобальный `ErrorBanner` модификатор

### Этап 7 — iOS: аутентификация

- [x] `LoginView` + `RegisterView`
- [x] `AuthViewModel` — login/register/logout
- [x] Хранение токенов в Keychain
- [x] Автоматическое обновление access token (interceptor)
- [x] Защищённый роутинг: переход на главную после логина

### Этап 8 — iOS: список и детали рецепта

- [x] `RecipeListView` — сетка карточек (LazyVGrid, 2/3/4 колонки)
- [x] `RecipeCardView` — компонент карточки с обложкой, названием, мета
- [x] Поиск (searchable + debounce)
- [x] Фильтр-шторка (категория, сложность, время)
- [x] Чипы активных фильтров
- [x] Пагинация (infinity scroll)
- [x] Pull-to-refresh
- [x] `RecipeDetailView` — hero-фото, мета, ингредиенты, превью шагов
- [x] Масштабирование порций (×0.5/×1/×2/×3)
- [x] Кэширование изображений (NSCache + disk)

### Этап 9 — iOS: режим приготовления

- [x] `CookingSessionView` — полноэкранный пошаговый режим
- [x] Слайдер фотографий шага (TabView + PageTabViewStyle + pinch-to-zoom)
- [x] Прогресс-бар и навигация (кнопки + swipe-жест)
- [x] `TimerService` — обратный отсчёт, пауза, звуковой сигнал
- [x] Блокировка автоблокировки экрана
- [x] Экран завершения («Готово!» + анимация)

### Этап 10 — iOS: Hands-Free режим

- [x] Запрос разрешения камеры (Info.plist + runtime prompt)
- [x] `AVCaptureSession` + `VNDetectHumanHandPoseRequest`
- [x] Определение жеста «открытая ладонь + свайп» (по дельте запястья)
- [x] Жест «сжатый кулак» (удержание 1 сек)
- [x] Жест «два пальца V»
- [x] Оверлей с индикацией распознанного жеста
- [x] Задержка между срабатываниями 1.5 сек
- [x] Настройка чувствительности в SettingsView
- [x] Автоотключение при сворачивании приложения

### Этап 11 — iOS: редактор рецептов

- [x] `RecipeEditorView` — форма основной информации
- [x] PhotosPicker: обложка рецепта (crop 16:9)
- [x] Список ингредиентов (добавить / удалить / reorder)
- [x] Список шагов с `StepEditorView` (drag-to-reorder)
- [x] `StepEditorView`: заголовок, описание, фото (до 5 шт), таймер
- [x] Автосохранение черновика в SwiftData каждые 30 сек
- [x] Валидация и публикация
- [x] Предупреждение при выходе с несохранёнными изменениями

### Этап 12 — iOS: настройки, категории, офлайн

- [x] `SettingsView` — адрес сервера, руки-free настройки, аккаунт
- [x] `CategoryView` — список рецептов по категории
- [x] Offline-режим: кэш просмотренных рецептов в SwiftData
- [x] Обработка `noConnection` — показ кэша + banner
...
### После MVP
- [x] Голосовые команды (Speech framework)
