...
- [y] `RecipeListView` — сетка карточек (LazyVGrid, 2/3/4 колонки)
- [y] `RecipeCardView` — компонент карточки с обложкой, названием, мета
- [y] Поиск (searchable + debounce)
- [y] Фильтр-шторка (категория, сложность, время)
- [y] Чипы активных фильтров
- [y] Пагинация (infinity scroll)
- [y] Pull-to-refresh
- [y] `RecipeDetailView` — hero-фото, мета, ингредиенты, превью шагов
- [y] Масштабирование порций (×0.5/×1/×2/×3)
- [y] Кэширование изображений (NSCache + disk)

### Этап 9 — iOS: режим приготовления

- [y] `CookingSessionView` — полноэкранный пошаговый режим
- [y] Слайдер фотографий шага (TabView + PageTabViewStyle + pinch-to-zoom)
- [y] Прогресс-бар и навигация (кнопки + swipe-жест)
- [y] `TimerService` — обратный отсчёт, пауза, звуковой сигнал
- [y] Блокировка автоблокировки экрана
- [y] Экран завершения («Готово!» + анимация)

### Этап 10 — iOS: Hands-Free режим

- [y] Запрос разрешения камеры (Info.plist + runtime prompt)
- [y] `AVCaptureSession` + `VNDetectHumanHandPoseRequest`
- [y] Определение жеста «открытая ладонь + свайп» (по дельте запястья)
- [y] Жест «сжатый кулак» (удержание 1 сек)
- [y] Жест «два пальца V»
- [y] Оверлей с индикацией распознанного жеста
- [y] Задержка между срабатываниями 1.5 сек
- [y] Настройка чувствительности в SettingsView
- [y] Автоотключение при сворачивании приложения

### Этап 11 — iOS: редактор рецептов

- [y] `RecipeEditorView` — форма основной информации
- [y] PhotosPicker: обложка рецепта (crop 16:9)
- [y] Список ингредиентов (добавить / удалить / reorder)
- [y] Список шагов с `StepEditorView` (drag-to-reorder)
- [y] `StepEditorView`: заголовок, описание, фото (до 5 шт), таймер
- [y] Автосохранение черновика в SwiftData каждые 30 сек
- [y] Валидация и публикация
- [y] Предупреждение при выходе с несохранёнными изменениями
